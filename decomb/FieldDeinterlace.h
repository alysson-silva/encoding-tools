...
public:
	...
	FieldDeinterlace(PClip _child,
					...
					IScriptEnvironment* env) : GenericVideoFilter(_child), full(_full),
					threshold(_threshold), dthreshold(_dthreshold), map(_map), blend(_blend), chroma(_chroma),
					ovr(_ovr), show(_show), debug(_debug)
	{
		FILE *f;
		int count = 0;
		char c, *d;
	...
	FieldDeinterlace::~FieldDeinterlace(void)
	{
