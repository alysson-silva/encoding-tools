...
void func_0(void *ps)
{
	...
	for (int y=sheight; y<eheight; y+=inc)
	{
		...
		{
			pss->proc0(srcp+x,hw,dftr,src_pitch,sbsize);
		...
		for (int x=0; x<=width-sbsize; x+=inc)
		{
			for (int z=0; z<=stopz; ++z)
				pss->proc0(pfplut[z]+x,hw+sbsize*sbsize*z,
					dftr+sbsize*sbsize*z,src_pitch,sbsize);
			pss->fftwf_execute_dft_r2c(pss->ft,dftr,dftc);
			if (zmean)
				pss->removeMean((float*)dftc,(float*)dftgc,ccnt,(float*)dftc2);
			pss->filterCoeffs((float*)dftc,pss->sigmas,ccnt,
...
{
	nlf->pf->copyFrom(child->GetFrame(mapn(n), env), vi);
	copyPad(nlf->pf,nlf->ppf,env);
	for (int b=0; b<3; ++b)
	{
		if ((b == 0 && !Y) || (b == 1 && !U) || (b == 2 && !V))
		{
			nlf->pf->copyPlaneTo(*dstPF, b);
			continue;
		}
		const int height = nlf->ppf->GetHeight(b);
		...
		float *ebp = ebuff[b]+width*((height-src_height)>>1)+((width-src_width)>>1);
...
PVideoFrame dfttest::GetFrame_T(int n, IScriptEnvironment *env)
{
	...
	if (tmode == 0)
	{
		for (int b=0; b<3; ++b)
		{
			if ((b == 0 && !Y) || (b == 1 && !U) || (b == 2 && !V))
				continue;
			memset(ebuff[b],0,src->GetHeight(b)*src->GetWidth(b)*sizeof(float));
		}
		pos = tbsize>>1;
		...
		for (int i=n-pos; i<=n+pos; ++i)
		{
			...
			if (nl->fnum != i)
			{
				nl->pf->copyFrom(child->GetFrame(mapn(i), env), vi);
		...
		else
		{
			...
			for (int q=tbsize-tosize; q<tbsize; ++q)
			{
				for (int b=0; b<3; ++b)
				{
					if ((b == 0 && !Y) || (b == 1 && !U) || (b == 2 && !V))
						continue;
					memset(ebuff[q*3+b],0,src->GetHeight(b)*src->GetWidth(b)*sizeof(float));
				}
			}
		}
	...
	for (int b=0; b<3; ++b)
	{
		if ((b == 0 && !Y) || (b == 1 && !U) || (b == 2 && !V))
		{
			fc->frames[fc->getCachePos(pos)]->pf->copyPlaneTo(*dstPF,b);
			continue;
		}
		...
		const int src_width = dstPF->GetWidth(b);
		const int src_height = dstPF->GetHeight(b);
		float *ebp = ebuff[tmode==0?b:(pos*3+b)]+width*((height-src_height)>>1)+((width-src_width)>>1);
		long cpuflags = env->GetCPUFlags();
		if (dither)
			dither_C(ebp,dstp,src_height,src_width,dst_pitch,width,dither);
		else if (!(src_width&7) && (((cpuflags&CPUF_SSE2) && opt == 0) || opt == 3))
...
void dfttest::copyPad(PlanarFrame *src, PlanarFrame *dst, IScriptEnvironment *env)
{
	for (int b=0; b<3; ++b)
	{
		...
		{
			env->BitBlt(dst->GetPtr(b)+dst->GetPitch(b)*y,dst->GetPitch(b),
				dst->GetPtr(b)+dst->GetPitch(b)*w,dst->GetPitch(b),dst->GetWidth(b),1);
		}
		w = offy+src_height-2;
...
dfttest::dfttest(PClip _child, bool _Y, bool _U, bool _V, int _ftype, float _sigma, 
	...
	IScriptEnvironment *env) : GenericVideoFilter(_child), Y(_Y), U(_U), V(_V), 
	...
	pmaxfile(_pmaxfile), opt(_opt), threads(_threads), dither(_dither)
{
	...
	if (!vi.IsYUY2() && !vi.IsYV12())
		env->ThrowError("dfttest:  only yv12 and yuy2 input supported!");
	if (ftype < 0 || ftype > 4)
	...
	if (smode == 0)
	{
		...
		noyl = vi.height+ae;
		noxc = (vi.width>>1)+ae;
		if (vi.IsYV12()) noyc = (vi.height>>1)+ae;
		else noyc = vi.height+ae;
	}
	else
	{
		const int ae = max(sbsize-sosize,sosize)*2;
		noxl = vi.width+EXTRA(vi.width,sbsize)+ae;
		noyl = vi.height+EXTRA(vi.height,sbsize)+ae;
		noxc = (vi.width>>1)+EXTRA(vi.width>>1,sbsize)+ae;
		if (vi.IsYV12()) noyc = (vi.height>>1)+EXTRA(vi.height>>1,sbsize)+ae;
		else noyc = vi.height+EXTRA(vi.height,sbsize)+ae;
	}
	PlanarFrame *padPF = new PlanarFrame();
	padPF->createPlanar(noyl,noyc,noxl,noxc);
	if (tbsize > 1)
		fc = new nlCache(tbsize,padPF,(VideoInfo)vi);
	else
		nlf = new nlFrame(padPF,(VideoInfo)vi);
	...
	for (int q=0; q<ebcount; ++q)
	{
		for (int b=0; b<3; ++b)
		{
			...
			ebuff[q*3+b] = (float*)_aligned_malloc(padPF->GetWidth(b)*
				padPF->GetHeight(b)*sizeof(float),16);
		}
	}
	delete padPF;
	...
	for (int i=0; i<threads; ++i)
	{
		...
		for (int b=0; b<3; ++b)
		{
			const int height = smode == 0 ? pf->GetHeight(b)-((sbsize>>1)<<1) : 
				(pf->GetHeight(b)-sosize)/(sbsize-sosize);
	...
	float *dest, const float wscale, IScriptEnvironment *env)
{
	...
	PlanarFrame *prf = new PlanarFrame(vi);
	...
	for (int ct=0; ct<nnpoints; ++ct)
	{
		...
		for (int z=0; z<tbsize; ++z)
		{
			...
			pss->proc0(srcp,hw2+sbsize*sbsize*z,dftr+sbsize*sbsize*z,pitch,sbsize);
		}
...
AVSValue __cdecl Create_dfttest(AVSValue args, void* user_data, IScriptEnvironment* env)
{
	return new dfttest(args[0].AsClip(),args[1].AsBool(true),args[2].AsBool(true),
		args[3].AsBool(true),args[4].AsInt(0),args[5].AsFloat(16.0),args[6].AsFloat(16.0),
		args[7].AsFloat(0.0f),args[8].AsFloat(500.0f),args[9].AsInt(12),args[10].AsInt(1),
		...
		args[22].AsString(""),args[23].AsString(""),args[24].AsFloat(1.0f),
		args[25].AsString(""),args[26].AsInt(0),args[27].AsInt(0),
		args[28].AsString(""),args[29].AsString(""),args[30].AsString(""),
		args[31].AsString(""),args[32].AsString(""),args[33].AsInt(0),env);
}
